...
\jtitle%
{An Evolutionary-Computing Based Framework for Approaching Simultaneous Continuous Games with Infinite Solutions}
...
\etitle{An Evolutionary-Computing Based Framework for Approaching Simultaneous Continuous Games with Infinite Solutions}
...
% In this paper, we present a novel evolutionary-computing based optimization procedure for continuous games of simultaneous decision, specifically aimed at providing a more comprehensive framework for approximating game solutions even when the objective space is continuous and may contain an infinite number of solutions.
...
% Through experimental validation with a cybersecurity game model, we demonstrate the method's convergence to a finite but representative number of game solutions, and discuss the many possible future improvements.
...
The goal of the analyst is to find the Nash Equilibria (NE) of the game, which are states (often considered ``game solutions'' \cite{bonanno2018game}) where all players successfully maximize their payoffs given the strategies of the other players.
This is not equivalent to treating a game model as a Multi-Objective Optimization Problem (MOOP), as the players are not cooperating towards a common global optimal, but rather are more prone to sabotage each other's efforts; for this reason, games require dedicated optimization methods.
...
To the best of our knowledge, no other numerical method had been demonstrated for solving continuous games of simultaneous decision with multi-objective players and an infinite number of solutions.
...
Beyond the concrete basic algorithmic implementation demonstrated in this work, the proposed method contributes a general abstract procedure, which we hope will guide the development of new, more efficient methods for solving game theory models involving simultaneous decisions, continuous decision spaces, and any number of players and solutions, with applications beyond FlipIt.
...
Games with simultaneous decisions by all players can be modeled by a quadruple $\left (I, (S_0, S_1, \dots, S_{n-1}), O, f \right )$ \cite{bonanno2018game}, comprised of a list of player identifiers $I = \{0, 1, \dots, n-1\}$ (where $n \geq 2$ is the number of players), a list containing one strategy set $S_p$ per player $p \in I$, a set $O$ of possible outcomes, and a payoff function $f$.
...
\noindent
where all $s_i \in S_i$ and all $s^*_j \in S_j$, and $\succeq_p$ is a relation such that $a \succeq_p b$ denotes that $p$'s perceived outcome from $a$ is preferred to that of $b$ ($a, b \in O$).
...
One should also note that, in an NE, no player regrets their choice even after knowing the decisions of the other players (given that all of them chose their strategy for that NE and are therefore playing a best response).
...
In the example, player 0's preferred payoff to each choice by player 1 is highlighted in blue and underscored; the same was done for player 1 in red and double underscore.
The PSNE are identified at the end of this process as the strategy profiles for which all players get their preferred payoff simultaneously (i.e., players are all playing a best response against each other).
In our example, the only PSNE is $(\texttt{Confess}, \texttt{Confess})$.

\begin{table}[h]
    \caption{Payoff matrix for the Prisoner's Dilemma; the only Nash Equilibrium is the highlighted cell.}
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        ...
        \hline
        \textbf{Silent} & {
            \colorbox{White!25}{\makebox[1em]{\strut -1}}, \colorbox{White!25}{\makebox[1em]{\strut -1}}
        } & {
            \colorbox{White!25}{\makebox[1em]{\strut -9}}, \colorbox{Red!25}{\makebox[1em]{\strut\doubleunderline{0}}}
        } \\
        \hline
        \textbf{Confess} &{
            \colorbox{Cyan!25}{\makebox[1em]{\strut\underline{0}}}, \colorbox{White!25}{\makebox[1em]{\strut -9}}
        } & \cellcolor{LimeGreen!50}{
            \colorbox{Cyan!25}{\makebox[1em]{\strut\underline{-5}}}, \colorbox{Red!25}{\makebox[1em]{\strut\doubleunderline{-5}}}
        } \\
        \hline
    \end{tabular}
...
In the original FlipIt game, two players ($I = \left\{ 0, 1 \right\}$) compete for control of a ``resource'', which is an abstraction for a computational resource which the defender player wishes to protect, such as a computer, a network or a password.
The attacker player, on the other hand, wishes to compromise it.
...
Each move incurs a cost ($k_i$) that negatively impacts $\beta_i$.
...
PS is fully characterized by a scalar move rate $\alpha_i$, which determines the time between moves by the adopting player.
As a consequence, a PS game of FlipIt can be modeled as a quadruple $\left (I=\left\{ 0,1 \right\}, (S_0 \subseteq \Rnn, S_1 \subseteq \Rnn), O \subseteq \R^2, f \right )$ \footnote{We use $\R$ to denote the set of real numbers, and $\Rnn$ for the set of non-negative real numbers.}.
The strategy sets represent each player's choices for $\alpha_i$, and the fact that they are infinite means that the simple, classic procedure presented in Subsection \ref{gametheory} is not applicable.
The best responses for this game were, instead, successfully determined algebraically \cite{dijk2013flipit}, resulting in analytical expressions for $f$ and $E$; however, the same is not true for more complex strategies for FlipIt, and/or for some of its extensions, and/or for scenarios where the opponents can choose from many different classes of strategies.
...
The main idea is that, for PS games of FlipIt with IE, the defender's strategy set becomes $S_0=\Rnn^2$ to accommodate a new decision variable, $\phi$ (the amount of IE), which impacts the resource resilience to attacks, but also incurs in a one-time cost of $c\phi$ monetary units, where $c$ is a game parameter much like $k_0$ and $k_1$ mentioned earlier.
The objective space is now $O = \R^3$, with an additional component accounting for the defender's new objective, maximizing $K(\phi) = -c\phi$ (i.e., minimizing monetary costs).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{figs/p30_o12_40x40x160_400x100g_71ff16f_01_psne_decision_space.eps}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{figs/p30_o12_40x40x160_400x100g_71ff16f_01_psne_objective_space.eps}
...
One of our goals while designing this procedure was to keep it simple, for example, by avoiding performance optimizations for now, so that both game theorists and researchers in Computational Optimization can immediately understand the principles behind it.
...
In the case of the Prisoner's Dilemma, the best response decision vector (BRDV) for player 0 is $(\texttt{Confess})$ for OSP $(\texttt{Silent})$, and also $(\texttt{Confess})$ for OSP $(\texttt{Confess})$.
Similarly, the BRDV for player 1 is always $(\texttt{Confess})$.
...
In our example, many people instinctively believe that $(\texttt{Silent}, \texttt{Silent})$ is the optimal solution, but we observe that neither player plays a BRDV in this profile, so both are naturally inclined to deviate from it.
In $(\texttt{Confess}, \texttt{Confess})$, however, both are playing a BRDV, so there is no expectation of a payoff increase for that OSP.
...
For instance, even though player 1 perceives $(\texttt{Silent}, \texttt{Confess})$ as the most advantageous strategy profile, player 1 has no actual tool to convince player 0 to play $(\texttt{Silent})$, as in this game the latter would always be better off playing $(\texttt{Confess})$.
As a consequence, the sole solution is $(\texttt{Confess}, \texttt{Confess})$, meaning that this is the only way rational players are expected to play under the premise that they are greedy.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{figs/p30_o12_40x40x160_400x100g_71ff16f_01_analytical_surfaces.eps}
...
To address these limitations while still confirming the theoretical foundations of the idealized procedure, we propose a numerical procedure that discretizes the OSPs for each player, then approximates the BRDVs for each OSP using a regular single-objective or multi-objective Evolutionary Algorithm (EA).
To do so, we divide the decision space into a grid with a user-specified number of divisions per decision variable, compute the grid centers, and use opponent-controlled variable values from these centers as OSPs.
...
The PSNEs are then computed as the points from each player that are within a small arbitrary distance $\epsilon$ from a point from every other player (i.e., the intersection of the approximated BRDVs).

The procedure overview is shown in Algorithm~\ref{alg:approximation}.
The algorithm receives as inputs the player data (such as the number of decisions and objectives), the upper and lower bounds for the decision variables, the number of grid divisions per decision variable, the number of outer loops to perform, the number of nested EA generations, and the intersection threshold $\epsilon$.

The algorithm first computes the fixed OSPs for each player, then instantiates an EA for each player and OSP.
Each of these EAs have the sole purpose of approximating the BRDVs by optimizing that player's objectives through evolutionary operators over the player's decisions (i.e., no changes are made to the OSP).
Thus, each nested EA will be single-objective if the respective player is single-objective, or multi-objective if the player is multi-objective.
The EAs are always made to evolve for a fixed number of generations before the algorithm proceeds to compute the PSNEs by intersecting approximated BRDVs from all players.
Specifically, any BRDV estimation from one player that is within a small $\epsilon$ distance from at least one BRDV estimation from every other player is considered a PSNE.
This can be seen as an approximation to the final step in the classic procedure for finding PSNEs presented in Subsection \ref{gametheory}, and closely resembles the NE interpretations discussed in the same subsection.
Also, this allows one to establish a hard limit on the number of PSNEs to be found, which is given by the grid dimensions and the EA population sizes.
...
\begin{algorithm}[h]
    ...
    \KwIn{\\
        \begin{tabular}{ll}
            ...
            \texttt{n\_nest\_gens} & Nested generations\\
    ...
    \KwOut{\\
        \begin{tabular}{ll}
            ...
            \texttt{fits} & Fitness values for approxs.\\
        \end{tabular}
    }
...
Game results were evaluated algebraically through the payoff functions for each player.
...
The intersection threshold $\epsilon$ was set to half the diagonal of a grid cell.

$(\mu+\lambda)$ Simple EA \cite{Coello_2013} was chosen for each single-objective BRDV approximation (i.e., each attacker EA, for FlipIt with IE), while NSGA-II \cite{Deb_2002} was used for the multi-objective ones (i.e., each defender EA).
Initial populations were uniformly distributed in each player's strategy set.
A population size of 30 was used in all EAs, with an offspring size of 10.
Parent selection used Binary Tournament.
Crossover was arithmetic, allowing extrapolation up to 0.05 distance units away from the parents.
The algorithm was executed for 400 generations, with each nested EA running 100 generations each time.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{figs/p30_o12_40x40x160_400x100g_71ff16f_01_population_decision_space.eps}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{figs/p30_o12_40x40x160_400x100g_71ff16f_01_population_objective_space.eps}
...
The numerical approximation procedure was designed to be very simple, meaning that many of the optimizations that could be performed to improve its performance were not implemented at this stage.
However, even this basic implementation was shown to be effective in finding a finite number of PSNEs for FlipIt with IE, a game with one single-objective player, one multi-objective player, and three continuous decision variables in total.
...
As future works, we plan to increase the method's computational efficiency by enhancing the intersection-computing algorithm and by dynamically allocating resources to the EAs (e.g., population size, offspring size, number of generations) based on criteria such as convergence and distance to opponent populations.
We also plan to fully investigate the effects of increasing the grid resolution, especially in a dynamic manner as the nested EAs start to converge and therefore to reveal the regions of interest in the decision space.
These improvements may allow for a finer grid, faster convergence, larger population sizes and, hopefully, a larger and more accurate set of estimations.
Initial experiments in this direction have already shown promising results.
Finally, we also plan to extend this method to be able to investigate adaptive strategies for FlipIt.
