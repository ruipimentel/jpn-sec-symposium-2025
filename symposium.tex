...
Another work \cite{leite2025jpnsectransaction} proposed a novel evolutionary-computing based optimization procedure for continuous games of simultaneous decision, specifically aimed at providing a more comprehensive framework for approximating game solutions by decomposing the decision space into a grid of MOOPs.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{figs/pop100x20_500x5g_tanned-P01-K01_01_9236fdb_22_ds.eps}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{figs/pop100x20_500x5g_tanned-P01-K01_01_9236fdb_22_os.eps}
...
In the Multi-Objective Evolutionary Algorithm (MOEA) grid-based framework \cite{leite2025jpnsectransaction} and the single-PSNE CoEA approach \cite{leite2025gecco}, MOEAs were used to solve player problems as part of the process of finding PSNE, guided by the Pareto-dominance relation.
...
When the non-dominated sorting concept present in the Non-Dominated Sorting Genetic Algorithm II (NSGA-II) \cite{Deb_2002} was used, the rank is computed based on traditional Pareto-dominance between solutions.
...
Leite et al. \cite{leite2025jpnsectransaction} define the Opponent Strategy Profile (OSP) for a given player as the strategy profile composed of the decision-variable values of all other players except the given player.
...
To evaluate the fitness for each candidate, we spawn one Evolutionary Algorithm (EA) per candidate and per player in the game.
...
In these situations, from \cite{leite2025jpnsectransaction}, we know that a strategy profile is a true PSNE if each player's strategy is an exact BRDV for the OSP defined by the other players' BRDVs simultaneously.
...
\begin{figure*}[h]
  \centering
  \includegraphics[width=0.65\textwidth]{./figs/flowchart_mane.eps}
  \caption{Comparison between the flow of NSGA-II (left) and the proposed TANNED approach (right).}
...
In contrast to traditional ranking concepts, which are based on instantaneous relations between individuals in terms of fitness values (e.g., traditional Pareto-dominance), we propose a new ranking concept called Topology-Agnostic Nearest-Neighbor Error Dominance (TANNED).
...
\begin{enumerate}
    ...
    \item Select the best individuals primarily by rank similarly to NSGA-II selection: add all elements from the lowest rank to the next generation's population until no more ranks can be added without exceeding the population size; then, select the remaining individuals from the next rank using Crowding Distance (CD).
\end{enumerate}
...
To evaluate the performance of the proposed TANNED approach, we ran experiments on FlipIt with IE, the same benchmark game used in \cite{leite2024cec}, \cite{leite2025jpnsectransaction} and \cite{leite2025gecco}.
...
Generational Distance (GD) \cite{lamont1999GD} is frequently used in evolutionary computation to measure the average distance from the estimations to the true Pareto-front.
Inverted Generational Distance (IGD) \cite{coello2004IGD} is another frequently used performance indicator that measures the opposite: the average distance from a sample of points of the true Pareto front to the generated estimations.
...
The results obtained with the TANNED approach are presented in Fig.~\ref{fig:tanned:results}, where they are compared to those obtained with the decision-variable CoEA \cite{leite2024cec} and the MOEA-grid approach \cite{leite2025jpnsectransaction}.
...
\begin{figure*}[htpb]
    \centering
    \includegraphics[width=.75\textwidth]{./figs/mane_non_zero_K_P.eps}
    \caption{Mean GDe and IGDe over generations for the TANNED approach, for runs with different values of $K \neq 0$ and $P \neq 0$; the interquartile ranges for previous approaches \cite{leite2024cec} \cite{leite2025jpnsectransaction} are shown as dashed lines.}
...
\cite{leite2025jpnsectransaction}, on the other hand, presents a range of IGDe values comparable to that of TANNED, but with an average execution time of 838.9 seconds per run, which is significantly higher than that of TANNED (257.6 seconds).
...
When $K=1$, the performance is generally good (except for $P=0$), with most configurations rivaling typical performances of \cite{leite2025jpnsectransaction} in terms of IGDe.
...
Author Leite acknowledges support by the Federal University of Technology - Paraná (UTFPR) and by the Japanese Ministry of Education, Culture, Sports, Science and Technology (MEXT).
