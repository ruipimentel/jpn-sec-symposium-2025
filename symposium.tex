...
Both can make moves at any point along a continuous timeline to seize control of the resource, with each move incurring costs that impact their overall payoff.
...
In this paper, we present a novel optimization procedure for continuous games of simultaneous decision, specifically aimed at providing a more comprehensive framework for approximating game solutions even when the objective space is continuous and may therefore potentially contain an infinite number of game solutions.
Through experimental validation, we demonstrate the method's convergence to a finite but representative number of PSNE, with noise decreasing as more computing resources are allocated to the optimization process.

Beyond the concrete basic algorithmic implementation demonstrated in this work, the proposed method contributes a refined abstract procedure, which we hope will guide the development of new, more efficient methods for solving game theory models involving simultaneous decisions, continuous decision spaces, and any number of players and solutions, with applications beyond FlipIt.
...
We can think of a strategy profile as the input vector for a game evaluation, which outputs the payoff vector containing one or more outcomes per player.

As an example, let us consider a famous discrete, single-objective game of simultaneous decision called the Prisoner's Dilemma, where each of two prisoners (the players) can choose between remaining silent or betraying the other prisoner (their accomplice) by cooperating with the authorities.
...
In the Prisoner's Dilemma, player 1 has two OSPs: player 2 is silent, or player 2 confesses.
